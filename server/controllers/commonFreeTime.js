...
let timetable_day = {
    "8.5" : 0, "9" : 0, "9.5" : 0, "10" : 0, "10.5" : 0, "11" : 0, "11.5" : 0, "12" : 0, "12.5" : 0, "13" : 0, "13.5" : 0, "14" : 0, 
    "14.5" : 0, "15" : 0, "15.5" : 0, "16" : 0, "16.5" : 0, "17" : 0, "17.5" : 0, "18" : 0, "18.5" : 0, "19" : 0, "19.5" : 0, "20" : 0, 
    "20.5" : 0, "21" : 0, "21.5" : 0, "22" : 0, "22.5" : 0, "23" : 0
}
...
const return_freeTime = async(req,res)=>{ // front end should call this function for each week
    const input_appointment_list = [[],[]];
    const week = 0;
    input_appointment_list = req.body.appointmentList;
    week = req.body.week; // week must be correct

    let result1 = [];
    let result2 = [];

    if (week == -1){
        let result2 = findFreeTime(input_appointment_list[1], 8);
    } else if (week == 0){
        let result2 = findFreeTime(input_appointment_list[1], 1);
    } else if (week == 13){
        let result1 = findFreeTime(input_appointment_list[0], 13);
    } else if (week != 14){
        let result1 = findFreeTime(input_appointment_list[0], week);
        let result2 = findFreeTime(input_appointment_list[1], week+1);
    }
    ...
    for (let d = 1; d < 7; d++){
        for (let t = 8.5; t <= 23; t = t + 0.5){
            if (!timeOccupied[d.toString()][t.toString()]){
                if (oldDay == 0){ // first free time slot
                    oldDay = d;
                    oldStartTime = t;
                    endTime = t + 0.5;
                    continue;
                } else if (d == oldDay && t == endTime){ // continue the last time slot
                    endTime = endTime + 0.5;
